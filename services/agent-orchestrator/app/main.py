...
RAG_MIN_SCORE = float(os.getenv("RAG_MIN_SCORE", "0.01"))  # Lowered to accept RRF scores
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:  # Más tiempo para advanced
            ...
            try:
                ...
                if results:
                    ...
                    return results
                logger.info("Advanced search returned no results, trying normal search")
            except Exception as exc:
                logger.warning(f"Advanced search failed, trying normal: {exc}")
            
            # Fallback a búsqueda normal (si advanced falló o no trajo resultados)
            resp = await client.post(
...
async def chat(req: ChatRequest) -> Dict[str, Any]:
    ...
    if openai_client:
        try:
            system = (
                "Eres un asistente experto que tiene ACCESO DIRECTO a los documentos del usuario.\n\n"
                "TU FUENTE DE VERDAD:\n"
                "- La sección 'Contexto recuperado de documentos' contiene la información REAL de los archivos del usuario.\n"
                "- NO necesitas que el usuario te 'pase' los documentos; ya los estás leyendo en el contexto.\n\n"
                "REGLAS ABSOLUTAS:\n"
                "1. SIEMPRE usa la información del contexto para responder, incluso si parece parcial.\n"
                "2. NUNCA digas 'necesito que me proporciones los documentos' o 'no tengo acceso'. YA TIENES ACCESO.\n"
                "3. Si el contexto menciona el tema de la pregunta, ASUME que es relevante y úsalo.\n"
                "4. Cita siempre la fuente: 'Según [archivo.pdf]...'.\n"
                "5. Si el usuario pregunta 'qué documentos tienes', lista los nombres de archivo que ves en el contexto.\n\n"
                "EJEMPLO DE COMPORTAMIENTO:\n"
                "Usuario: 'Dame los créditos del máster'\n"
                "Contexto: '[guia.pdf] El máster consta de 60 ECTS...'\n"
                "Tu respuesta: 'Según [guia.pdf], el máster tiene 60 ECTS.' (NO digas 'necesito ver la guía')\n\n"
                "Si el contexto NO tiene NADA que ver (ej: pregunta de cocina, contexto de leyes), entonces responde con tu conocimiento general sin mencionar los documentos."
            )
